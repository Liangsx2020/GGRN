...
We propose G-GRN (Graph-based Generalized Regression Network) to fill this gap. G-GRN treats precomputed GFD stencil coefficients as edge weights in a graph neural network, replacing autograd with scatter-based derivative approximation. Because derivative accuracy now depends on stencil order rather than network smoothness, the method naturally benefits from mesh refinement, a property that standard PINNs lack. Training minimizes a strong-form loss combining the PDE residual, Dirichlet boundary conditions, and both interface transmission conditions ($[u]$ and $[\beta\partial_n u]$). Experiments on three interface geometries (circular, oscillating, elliptic) confirm that G-GRN outperforms standard PINNs in both accuracy and training speed.
...
Treating all edges uniformly is insufficient for interface problems, so we partition $\mathcal{E}$ into two disjoint subsets based on the region indicator. \textit{Homophilous edges} connect nodes within the same subdomain ($z_i = z_j$); they represent connectivity within a continuous medium and are used exclusively for GFD stencil aggregation. \textit{Heterophilous edges} connect nodes across the interface ($z_i \neq z_j$); because differentiation across the discontinuity is ill-posed, these edges are excluded from the network's message-passing layers and appear only in the loss function, where they define the node pairs for evaluating the interface jump conditions (Section~\ref{sec:loss}). This partition is encoded in a binary edge attribute $e_{ij} \in \{0, 1\}$. The combination of dense near-interface sampling and explicit edge classification provides the spatial resolution needed to evaluate flux jumps accurately where the solution is least smooth.
